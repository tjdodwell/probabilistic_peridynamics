...
class param:
	...
	def findBoundary(self,x):
	    ...
	    if (x[0] < (self.tabLength + 1e-6)):
	        bnd = -1
	    elif (x[0] > (self.totalLength - self.tabLength - 1e-6)):
	        bnd = 1
	    return bnd
	...
	def isCrack(self, x, y):
		...
		output = 0
		tL = self.totalLength
		if ((p1[0] < 0.5 * tL) and (p2[0] > 0.5 * tL)):
			m = (p2[1] - p1[1]) / (p2[0] - p1[0]) # = tan(theta)
			h = (p1[0] - 0.5 * tL) * m
			if( h > self.totalWidth - self.notchDepth):
				output = 1
		return output
...
for t in range(1, numSteps):
	...
	u[t][myModel.lhs,0] = - 0.5 * time * loadRate * np.ones(len(myModel.rhs))
	u[t][myModel.rhs,0] = 0.5 * time * loadRate * np.ones(len(myModel.rhs))


	# Write Solution to file every N steps
	if ((t % print_every == 0)):
		vtu.writeParallel("U_" + str(t),comm, myModel.numlocalNodes, myModel.coords[myModel.l2g,:], d[t], u[t][myModel.l2g,:])
