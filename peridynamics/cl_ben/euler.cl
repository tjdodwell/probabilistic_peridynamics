...
__kernel void
     ...
     * nlist - An (n, local_size) array containing the neighbour lists,
     *     a value of -1 corresponds to a broken bond.
     * stiffness_correction_factors - An (n * max_neigh) array of bond stiffness correction factors.
     * material_types - An (n * max_neigh) array of m material types.
     * stiffnesses - An (m, PD_REGIMES_NO) array of the different bond stiffnesses for m material types, 
     *      where PD_REGIMES_NO the number of different stiffnesses in the force density function e.g.
     *      PD_REGIMES_NO = 1 : PMB/ linear damage model, PD_REGIMES_NO = 2 : bilinear damage model, etc.
     * critical_stretches - An (m, PD_REGIMES_NO) array of different critical strains for m material types.
     * fc_types - An (n,3) array of force boundary condition types...
     *     a value of 2 denotes a particle that is not externally loaded.
     * fc_values - An (n,3) array of the force boundary condition values applied to particles.
     * local_cache_x - local (local_size) array to store the x components of the bond forces.
     * local_cache_y - local (local_size) array to store the y components of the bond forces.
     * local_cache_z - local (local_size) array to store the z components of the bond forces.
     * force_load_scale - scale factor applied to the load conditions*/
    // global_id is the bond number
    const int global_id = get_global_id(0);
    // local_id is the LOCAL node id in range [0, max_neigh] of a node in this parent node's family
	const int local_id = get_local_id(0);
	...
	if ((global_id < (PD_NODE_NO * local_size)) && (local_id >= 0) && (local_id < local_size)) {  
        ...
        if (node_id_j != -1) {
            ...
            local_cache_y[local_id] = force_density * volume * cy;
            local_cache_z[local_id] = force_density * volume * cz;
...
__kernel void
	...
	if ((i < PD_NODE_NO) && (j >= 0) && (j < MAX_HORIZON_LENGTH)) {
		const int n = nlist[i * MAX_HORIZON_LENGTH + j];
		...
		if (n != -1) {
			...
			const double xi_eta_x = u[DPN * n + 0] - u[DPN * i + 0] + xi_x;
			const double xi_eta_y = u[DPN * n + 1] - u[DPN * i + 1] + xi_y;
			const double xi_eta_z = u[DPN * n + 2] - u[DPN * i + 2] + xi_z;

			const double xi = sqrt(xi_x * xi_x + xi_y * xi_y + xi_z * xi_z);
			const double y = sqrt(xi_eta_x * xi_eta_x + xi_eta_y * xi_eta_y + xi_eta_z * xi_eta_z);

			const double s0 = critical_stretches[i * MAX_HORIZON_LENGTH + j];
			...
			if (s > s0) {
				nlist[i * MAX_HORIZON_LENGTH + j] = -1;  // Break the bond
			}
		}
	}
}