def write(fileName, numPoints, x, damage, U):
    ...
    f.write("\t\t\t<DataArray type=\"Float32\" Name=\"Damage\""
            " Format=\"ascii\">\n")
    for i in range(0, numPoints):
        f.write("\t\t\t\t %f\n" % damage[i])
    f.write("\t\t\t</DataArray>\n")
    f.write("\t\t\t<DataArray type=\"Float32\" Name=\"Displacement\""
    ...
    for i in range(0, numPoints):
        f.write("\t\t\t\t %s" % (i + 1))
    f.write("\t\t\t</DataArray>")
    f.write("\t\t\t<DataArray type=\"Int32\" Name=\"types\""
            " Format=\"ascii\">\n")
    for i in range(0, numPoints):
        f.write("\t\t\t\t 1\n")
    f.write("\t\t\t</DataArray>\n")
...
def writeParallel(fileName, comm, numPoints, x, damage, U):
    ...
    if(rank == 0):
        ...
        f.write("\t\t<PPoints>\n")
        f.write("\t\t\t<PDataArray type=\"Float32\" Name=\"Coordinates\""
                " NumberOfComponents=\"3\"/>\n")
        f.write("\t\t</PPoints>\n")
        f.write("\t\t<PCellData>\n")
        f.write("\t\t</PCellData>\n")
        f.write("\t\t<PPointData Scalars=\"Damage\""
                " Vectors=\"Displacement\">\n")
        f.write("\t\t\t<PDataArray type=\"Float32\" Name=\"Damage\""
                " NumberOfComponents=\"1\"/>\n")
        f.write("\t\t\t<PDataArray type=\"Float32\" Name=\"Displacement\""
                " NumberOfComponents=\"3\"/>\n")
        f.write("\t\t</PPointData>\n")
        for i in range(0, nproc):
            f.write("\t\t<Piece Source=\"" + fileName + "_proc_" + str(i)
                    + ".vtu\"/>\n")
