...
class MultiscaleGrid:
    ...
    def buildCoarseGrid(self, type, L, n, X0, hf):
        # buildCoarseGrid constructs an indentical coarse grid on each
        # processor it then builds a local mesh for each subdomain - including
        # overlap Identical Coarse Grid is built on each processor
        self.macroGrid = fem.Grid()

        self.macroGrid.buildStructuredMesh2D(L, n, X0, 1, 0)

        # Type: element or node centric
        # Node centric - Overlapping, each domain support of node (assumming
        # linear macro finite element)
    ...
    def buildPartitionOfUnity(self):
        # In this case we will assume POU is linear shape functions on
        # macroGrid
        # Setup vector to store POU - xi
        self.xi = np.zeros(self.fineGrid.numNodes)
        ...
        for i in range(0, self.fineGrid.numNodes):
            ...
            if self.dim == 2:
                ...
                if d[0] > 0 and d[1] > 0:
                    theNode = 0
                    localX[0] = ((d[0] - 0.5 * self.macroGrid.h[0])
                                 / self.macroGrid.h[0])
                    localX[1] = ((d[1] - 0.5 * self.macroGrid.h[1])
                                 / self.macroGrid.h[1])
