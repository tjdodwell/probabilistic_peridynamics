...
def cython_model(data_path, request):
...
def cl_model(data_path, request):
    """Create a simple peridynamics Model and ModelCL object."""
    path = data_path
    mesh_file = path / "example_mesh.vtk"
...
@pytest.fixture(scope="session")
def simple_displacement_bc_array():
    """Return a simple displacement boundary magnitudes function."""
    def displacement_bc_function(steps, max_displacement):
        """Return a simple displacement boundary function."""
        displacement_bc_array = np.linspace(steps, max_displacement, steps)
        return displacement_bc_array
    return displacement_bc_function
