...
def decomposeDomain(coords, connectivity, M, partitionType):
    ...
    Inputs: node coords, connectivity matrix,
    number of processes (M)

    Output: partition (array length nnodes of which paritition each node is
    in), nearestNeighbour (a list of lists, containing the partition neighbours
    for each partition, e.g. in X only, [[1], [0, 2], [1, 3], ..., [M-2]]
    """
    part = []
    if partitionType == 1:
        # Simple Domain Decomposition for now in X only - Probably works well
        # for examples considered
        maxX = np.amax(coords[:, 0])
        dX = (maxX / M) + 1e-6
        for i in range(0, coords[:, 0].size):
            part.append(np.floor(coords[i, 0] / dX))

        # Constructs nearest neighbour list for each subdomain. Simple in this
        # case
        nearestNeighbour = []
