...
from PostProcessing import vtk, vtu
...
class ParModel:
    def __init__(self):
        ...
        # List for Solution at each time step
        self.u = []
        # List containing damage at each time step
        self.damage = []

    def checkBonds(self, U, broken, damage):
        """
        Check bonds to see if they have been broken in this time step This is
        the same as the sequential code apart from we only loop over local
        particles

        Input: 'U' Particle displacements. 'U' will contain the displacements
        from the ghost particles becuase of communication step at beginning of
        the timestep 'broken' list of lists 'damage' list
        ...
        for i in range(0, self.numlocalNodes):
            ...
            for k in range(0, len(family)):
                ...
                # if bond is not previously broken
                if broken[i][k] == 0:
                    # deformed coordinates of particle j
                    yj = self.coords[j, :] + U[j, :]
                    bondLength = func.l2norm(self.coords[id_, :]
                                             - self.coords[j, :])
                    rvec = yj - yi
                    r = func.l2norm(rvec)
    ...
    def computebondForce(self, U, broken):
        ...
        F = np.zeros((self.numlocalNodes, 3))
        # For each particles in this subdomain
        for i in range(0, self.numlocalNodes):
            id_ = self.net[i].id
            yi = self.coords[id_, :] + U[id_, :]
    ...
    def readMesh(self, fileName):
        """
        Reads particle coordinates from the mesh, and mesh element
        connectivities. Domain decomposition is also done here.
        ...
        if f.mode == "r":
            ...
            if self.plotPartition & self.comm.Get_rank() == 0:
                vtk.write("Partition.vtk", "Partition", self.coords,
                          self.partition, np.zeros(self.nnodes, 3))
        f.close()

    def setNetwork(self, horizon):
        """
        Creates local to global id lists, calculates volumes of the particles,
        creates family lists and lists of ghost particles. Constructs the
        covariance matrix, K.
        ...
        for i in range(0, self.nnodes):
            ...
            if myRank == int(self.partition[i]):
                ...
                # This is the case where particle lives on another process but
                # is in a neighbouring subdomain, so has potential to be in
                # family
                check = 0
                for k in range(0, len(self.NN[myRank])):
                    if self.NN[myRank][k] == int(self.partition[i]):
                        check += 1
                if check > 0:
                    # Number greater than zero indicates that particle is in
                    # one of the neighbouring subdomains
        ...
        # Not ideal for now - but do on all processors over all elements -
        # saves dealing with boundary cases
        ...
        for i in range(0, self.numlocalNodes):
            ...
            for j in range(0, self.numlocalNodes):
                ...
                if globalId_i != globalId_j:
                    if func.isNeighbour(self.coords[globalId_i, :],
                                        self.coords[globalId_j, :], horizon):
                        testCrack = self.isCrack(self.coords[globalId_i, :],
                                                 self.coords[globalId_j, :])
                        if testCrack == 0:
                            tmp.append(globalId_j)

            # loop over only nodes in the nearest neighbour partitions
            for k in range(0, len(neighbour_ids)):
                if func.isNeighbour(self.coords[globalId_i, :],
                                    self.coords[neighbour_ids[k], :], horizon):
                    testCrack = self.isCrack(self.coords[globalId_i, :],
                                             self.coords[globalId_j, :])
                    if testCrack == 0:
        ...
        # Need to setup information to be communicated - THIS NOT THE WAY TO DO
        # IT :)
        ...
        if self.comm.Get_size() == 1:
            ...
            for i in range(0, self.comm.Get_size()):
                # Create Vector to mark which are neighest neighbours for
                # processor i
                ...
                if myRank == i:
                    for k in range(0, len(self.NN[i])):
                        # Mark as a neighest neighbour for processor i
                        areNN[self.NN[i][k]] = 1
                ...
                for k in range(0, len(self.NN[i])):
                    ...
                    if myRank == i:
                        # Create a list of ghost which live on a give
                        # neighbour
                        tmp = []
                ...
                # Processors Idle before we move to next processor
                self.comm.Barrier()

            if self.testCode:
                # Dirty hack as my quick vtkWriter only works for lists for
                # scalar variables
                data = []
                for i in range(0, self.nnodes):
                ...
                print('data_local length', len(data_local), 'numlocalnodes',
                      self.numlocalNodes, 'data length', len(data),
                      'l2g length', len(self.l2g))
    ...
    def communicateGhostParticles(self, u):
        """
        Code carries out communication of displacements required at the
        beginning of each step
        Carried out by exploiting nearest neighbour communicaiton as set up in
        setNetwork()
